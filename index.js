...
function getWinners(cb,data) {
    ...
    games.map(game=>{
        if(game["Home Team Goals"] === game{"Away Team Goals"}){
            let winner = game["Win conditions"].subst(0,game["Win conditions"].indexOf("w"));
            winners.push(winner);
        }else if(game["Home Team Goals"] > game["Away Team Goals"]){
            winners.push(game["Home Team Goals"] . game["Away Team Goals"])
        }else {
...
function getWinnersByYear(cb1, cb2) {
    ...
    for(let i = 0; i < winners.length; i++){
        allWinners.push(`In ${years}, ${winners[i]} won the world cup!`);
    }
    return allWinners;
};
...
function getCountryWins(data, intials) {
